...
export const notes = [
  ...
  ['и так, далее - разборки с ChangeDetection и как этот процесс отслеживать.',
    'возьму объект вида {name:"xx", pic:["img1", {s:"x.com", t:"title"}] }',
    'для него сделаю интерпретатор, которым можно будет менять значения кодом.',
    'логгирование и 2 компонента, которые будут его отображать с разными стратегиями.',
    {
      t: 'собственно вот так:',
      c: Inline02Component
    },
    {
      t: 'вот - как и было сказано в документации - по событию внешнему идёт проверка всего дерева на изменения:',
      s: ['сейчас поправил InlinerComponent что-бы он перезагружал требуемый компонент.',
        'пока проверял - загужая и меняя компонент выше (из заметки_17) увидел несколько лишних логов проверки AC:DoCheck.',
        'т.е. я уже сделал Inline02Component с логом и первым Inline02ActiveContainerComponent.']
    }],

  ['закончил с примером из с_6',
    {
      t: 'какие выводы по результатам сделать могу?',
      s: ['увидел событие OnDestroy по удалению элемента из списка.',
        'увидел множественные события DoCheck - они ориентир проверок выполняемых angular-ом.',
        'есть некоторая специфика DoCheck которая видна тут.',
        'увидел как можно вытянуть из проверок установкой другой стратегии.']
    },
    'т.е. основной результат - попробовал в тесте, понял что с этим могу работать,' +
    ' дальше возникает вопрос применения в проекте соответствующего - о котором ещё подумать надо.']
];
